...
import CalculatorBill from "@/modules/bill/function/CalculatorBill.ts";
...
const TableBill = React.memo(() => {
	...
	return <>
		...
		{bills.length > 0 && <div className={`mx-40 p-4 shadow-3 mb-4`}>
            ...
            <p className={`flex-between`}>
                ...
                <span className={`text-lg font-bold`}><NumberFormatter number={CalculatorBill(bills)}/></span>
            </p>
        </div>}
			...
			bills?.length === 0 ? <Empty className={`mt-20`}/> :
				<div className={`grid relative w-full md:grid-cols-2 gap-8`}>
					{bills?.map((el) => {
						...
						return <div key={el.id} className={`flex gap-10 shadow-3 p-8`}>
